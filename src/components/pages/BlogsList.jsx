...
>
  <div className="w-full md:w-3/4 text-left relative z-10">
    ...
    <div className="mt-4 w-full">
      <span className="text-secondary-300 lg:mx-16 lg:w-full text-body-2 font-grotesk block">
  ...
  <div className="w-full max-w-7xl mt-16 grid grid-cols-1 lg:grid-cols-4 gap-8 relative z-10">
      ...
      >
        <div className="relative w-full h-full bg-background-100 rounded-3xl overflow-hidden border border-secondary-600 hover:border-primary-300 transition-all duration-300">
          <div className="absolute inset-0.5 bg-background-100 rounded-[1.4rem] overflow-hidden z-0">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-secondary-500/50 via-secondary-400/10 to-transparent"></div>
          </div>

          <div className="relative z-10 p-8 lg:p-10 h-full flex flex-col">
            <div className="flex-grow">
              <div className="flex items-center space-x-2 mb-4">
                <span className="bg-primary-300 bg-opacity-20 backdrop-blur-sm text-primary-300 px-3 py-1 rounded-full text-body-5 font-mono uppercase tracking-wider border border-primary-300/40">
                  Featured
                </span>
                <span className="bg-secondary-500 bg-opacity-20 backdrop-blur-sm text-secondary-300 px-3 py-1 rounded-full text-body-5 font-mono uppercase tracking-wider border border-secondary-500/40">
                  {featuredBlog.category}
                </span>
              </div>
              <h2 className="text-heading-3 font-open-sans mb-4 text-light-100 ">{featuredBlog.title}</h2>
              <p className="text-light-100/70 font-grotesk pr-16 mb-8 line-clamp-3 text-body-2">{featuredBlog.summary}</p>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <div className="text-body-3">
                  ...
                  <p className="text-light-100/70">{new Date(featuredBlog.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
              <div className="flex items-center space-x-3 text-body-3 pr-12 lg:pr-10 text-light-100/70">
    ...
    {/* Categories Section */}
    <div className="lg:col-span-1 bg-gradient-to-br from-light-200 to-accent-200 rounded-3xl p-6 border border-light-100/60 shadow-sm backdrop-blur-md">
      <h3 className="text-body-1 font-grotesk font-medium mb-7 text-on-light relative inline-block">
        Categories
        <span className="absolute -bottom-1 left-0 w-12 h-0.5 bg-accent-400 rounded-full"></span>
      </h3>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        {CATEGORIES.slice(0, 2).map((category, index) => (
          <button 
            ...
            className={`group px-3 py-3.5 rounded-2xl text-body-3 transition-all duration-300 transform hover:scale-105 ${
              selectedCategory === category 
                ? 'bg-gradient-to-br from-primary-100 to-accent-100 text-primary-400 ring-1 ring-primary-300 shadow-sm' 
                : 'bg-light-100/80 text-on-light hover:bg-light-100 hover:shadow-sm'
            }`}
          >
            <div className="flex flex-col items-center">
              <div className={`w-10 h-10 rounded-full mb-2 flex items-center justify-center transition-colors duration-300 ${
                selectedCategory === category ? 'bg-primary-200' : 'bg-light-200 group-hover:bg-light-100'
              }`}>
                {getIconForCategory(category)}
              </div>
              <span className="font-open-sans text-body-5 tracking-wide">{category}</span>
              {selectedCategory === category && (
                <span className="h-0.5 w-6 mt-2 rounded-full bg-primary-300"></span>
              )}
            </div>
          </button>
        ))}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {CATEGORIES.slice(2, 5).map((category, index) => (
          <button 
            key={index + 2} 
            onClick={() => setSelectedCategory(category)}
            className={`group px-2 py-3.5 rounded-2xl text-body-3 transition-all duration-300 transform hover:scale-105 ${
              selectedCategory === category 
                ? 'bg-gradient-to-br from-primary-100 to-accent-100 text-primary-400 ring-1 ring-primary-300 shadow-sm' 
                : 'bg-light-100/80 text-on-light hover:bg-light-100 hover:shadow-sm'
            }`}
          >
            <div className="flex flex-col items-center">
              ...
              <span className="font-open-sans text-body-5 tracking-wide">{category}</span>
              {selectedCategory === category && (
                <span className="h-0.5 w-5 mt-2 rounded-full bg-primary-300"></span>
              )}
            </div>
          </button>
        ))}
  ...
  <div className="w-full max-w-7xl mt-20 relative z-10">
    <div className="flex justify-between items-center mb-10 border-b border-secondary-600 pb-4">
      <h2 className="text-heading-3  text-light-100 font-grotesk font-medium flex items-center">
        <span className="w-2 h-8 bg-primary-300 mr-3 rounded-full"></span>
        {selectedCategory === 'All' ? 'All Blogs' : `${selectedCategory} Blogs`}
      </h2>
      <div className="text-secondary-300 font-mono text-body-5">
        ...
        >
          ...
          <div className="px-1">
            <div className="flex items-center space-x-3 mb-3">
              <span className="bg-secondary-500/10 text-secondary-300 px-3 py-1 rounded-full text-body-5 font-mono uppercase tracking-wider">{blog.category}</span>
              <span className="text-secondary-300 text-body-5 font-mono">{blog.readTime} min</span>
            </div>
            ...
            <p className="text-secondary-300 font-grotesk line-clamp-2 text-body-2">{blog.summary}</p>
          </div>
        </Link>
      ))}
    </div>
    ...
    {filteredBlogs.length === 0 && (
      <div className="flex flex-col items-center justify-center py-16 text-secondary-300">
        ...
        <p className="text-body-1 font-mono">No blogs found in this category</p>
      </div>
    )}
